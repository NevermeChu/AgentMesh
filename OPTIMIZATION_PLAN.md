...
- **做法**：
  - executeCommand 子环境改白名单模式：基线集（PATH/HOME/USERPROFILE/TEMP/LANG/TZ/SYSTEMROOT…）+ 调用方 override 仅收白名单键，越键丢弃并计 warning（`envOverrideRejected:[...]`）；永久黑名单 LD_PRELOAD/NODE_OPTIONS/PYTHONPATH/DOCKER_HOST/KUBECONFIG/AWS_*。
