...
- Handoff grounding check: recorded turns verify every handoff `artifacts.files` claim against the recorded repository changes and the working tree; unlocatable claims surface a `Handoff grounding` warning on the MCP response (warning-level only, never alters turn status or the history schema), productizing the documented "test artifacts never landed in the workspace" failure mode.
- Typed exact-match token engine for lossless-handoff verification: `extractTypedTokens` / `diffTypedTokens` (paths, versions, commands, counts, hashes) and `findUngroundedHandoffFiles` are exported library primitives; the evaluation toolchain (`scripts/export-golden-trace.mjs`, `scripts/lossless-gates.mjs`) builds the offline structural/evidence/semantic gates over golden traces with the behavioral gate explicitly deferred to a live differential round.
- `REWOUND` context freshness: a working tree matching an EARLIER recorded turn (post-revert/reset) is now distinguished from `STALE`; the exhaustive `FRESHNESS_LABELS` record, injection labels, and sufficiency verdicts cover all four states.
- Structured `CONTEXT_INSUFFICIENT` misses: `get_session_context` returns `code` + `missing` for unknown sessions, empty histories, and out-of-range indexes, and successful retrievals carry a `sufficiency` verdict (`VERIFY_REQUIRED`/`INSUFFICIENT` with reasons) whenever freshness is not `MATCHED`, with matching `Context Status:` lines in the MCP text.
- Known failed attempts are rendered explicitly in multi-source injections (`Known failed attempts (do not repeat):`), listed as a low-priority droppable section recorded in `droppedSections`.
- `docs/real-test-spec-template.md`: mandatory SPEC clauses (untrusted-key protection, ordering semantics, undefined-zone policy, boundary semantics, round-trip declaration, anti-blindness self-test block) derived from the R20 real-test campaign findings.
- Structured handoff blockers: the Handoff Report contract gains a `## Blockers` section (`- <what is blocked> (requires: agent | user | resource | dependency | environment)`), parsed into a machine-readable `blockers: { summary, requires }[]` field on `HandoffSummary` (unparsable markers degrade to `user`, never dropped); injections render a decision-class `Blockers` section, and the MCP digest appends `blockers=N` plus an escalation line listing the distinct `requires` targets so orchestrators can route blockers without parsing prose.
- Knowledge-level staleness: repository-fingerprint freshness is now applied per turn — `get_session_context` returns `turnFreshness` + `turnSufficiency` for the retrieved turn itself (a REWOUND session can expose an earlier MATCHED turn safe to reuse alongside a superseded STALE latest turn), and injections prefix knowledge sections (Decisions/Findings/Blockers) with `[stale knowledge: …]` when the latest turn's recorded tree no longer matches the current one; no marker is fabricated when current evidence is unavailable. `computeTurnFreshness` is exported as a library primitive.
- Session event projection with an incremental cursor: turns are deterministically projected into an append-only event stream (`turn.recorded`/`handoff.recorded`/`findings.recorded`/`evidence.recorded`/`safety.recorded`, indexes and small actionable fields only — no separate mutable event store), exposed via the new `get_session_events` MCP tool (`afterEventId` exclusive cursor, `limit`, structured `CONTEXT_INSUFFICIENT` misses, `totalTurns` for trim detection) and the exported `projectSessionEvents` primitive, so orchestrators can poll what changed instead of replaying whole sessions.
...
- Shared-context retrieval guidance is orchestrator-mediated: the injected instruction asks receivers to declare missing upstream detail and request retrieval via the orchestrator instead of telling vendor agents to call `get_session_context` themselves — a channel the receiving vendor CLI does not have (R19 differential-round finding).
- Split the README into a concise bilingual README (English default + `README.zh-CN.md`) and moved all technical detail to `docs/GUIDE.md`.

### Fixed

- The external resource sampler's orphan check now requires pipeline connectivity (the candidate was sampled from the seed's tree, or its ancestry reaches the seed / an observed-tree pid through dead-but-observed hops); same-window regex matches outside the measured pipeline are counted in `orphans.unrelatedSameWindowMatches` instead of being reported as orphans (harness tooling whose paths embed the vendor name, e.g. `.zcode\...` session directories, previously self-matched the default vendor regex).
