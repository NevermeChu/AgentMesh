...
本指南承接 [README](../README.md) 的全部技术细节：核心特性、边界、Agent 矩阵、CLI 与角色配置、MCP Tools 完整参数、架构、测试与无损交接校验工具链。README 提供英文 / 简中双语精简版；本指南暂只维护中文。
...
1. **`delegate_task`**
   ...
   - 参数：`task` (必填), `agent` (可选，省略时读取项目角色映射), `cwd` (可选), `role` (可选: `worker` | `reviewer` | `tester`), `mode` (可选: `auto` | `mcp` | `cli`), `timeoutMs` (可选，最大 3600000), `model` (可选，vendor 专属模型标识，最长 200 字符), `reasoningEffort` (可选: `none` | `low` | `medium` | `high` | `xhigh`，是否生效取决于所选 vendor 与能力矩阵，见下文说明), `sessionId` (可选), `contextSessionIds` (可选，最多 4 个，按给定顺序一手注入多个 Bridge Session 的规范化历史), `contextSessionId` (可选，单源兼容形式), `baseCommit` (可选)。
2. **`review_changes`**
   - 调度指定 Agent 执行只读代码审查，强制遵循独立审查 Prompt 并返回结构化 PASS/FAIL 结果；PASS 可附带 medium/low 非阻塞 findings（critical/high 仍判失败）。
   - 参数：`agent` (可选，省略时读取 `roles.reviewer`), `task` (可选), `cwd` (可选), `baseCommit` (可选), `mode` (可选), `timeoutMs` (可选，最大 3600000), `model` (可选，vendor 专属模型标识，最长 200 字符), `reasoningEffort` (可选: `none` | `low` | `medium` | `high` | `xhigh`，是否生效取决于所选 vendor 与能力矩阵，见下文说明), `contextSessionIds` (可选，最多 4 个，如同时注入 Worker 与 Tester 的结论), `contextSessionId` (可选，单源兼容形式)。
3. **`continue_task`**
   - 继续已有会话（Session Resume），并可同时注入其他会话的上下文。
   - 参数：`sessionId` (必填), `task` (必填), `contextSessionIds` (可选，最多 4 个，与该会话自身的历史续接并存，例如一手注入 Reviewer/Tester 的反馈), `mode` (可选), `timeoutMs` (可选，最大 3600000), `model` (可选，vendor 专属模型标识，最长 200 字符), `reasoningEffort` (可选: `none` | `low` | `medium` | `high` | `xhigh`，是否生效取决于所选 vendor 与能力矩阵，见下文说明)。
4. **`list_agents`**
...
6. **`get_session_context`**
   - 按轮读取一个 Bridge Session 的记录内容：默认返回该轮的结构化 handoff（goal/decisions/artifacts/blockers/open items），也可按 `fields` 取回完整 `finalAnswer`、`findings` 或执行证据，并附当前工作树下的 `MATCHED`/`REWOUND`/`STALE`/`UNKNOWN` 新鲜度。这是 handoff 优先注入的按需取回通道：下游需要上游细节时调用本工具，而不是要求 AgentMesh 把全文灌进每个 prompt。
   ...
   - 返回还包含**逐轮独立的新鲜度** `turnFreshness`（与可选的 `turnSufficiency` 判定）：它衡量**该轮自身**记录的工作树指纹与当前树的关系，独立于会话级 `freshness`。会话级 `REWOUND` 时二者会出现关键分叉——被回退到的早轮是 `MATCHED`（结论可安全复用），而最新轮是 `STALE`（其结论已被树回退作废）；下游据此精确判断"哪一轮的知识仍然成立"，而不是只拿到会话级的单一结论。
...
7. **`get_session_events`**
   - 读取一个 Bridge Session 的**append-only 事件投影**（按轮派生的 `turn.recorded` / `handoff.recorded` / `findings.recorded` / `evidence.recorded` / `safety.recorded` 事件序列，payload 只含索引与小型可操作字段，如 blocker 的 `requires` 目标、findings 计数与严重级别、证据指纹）。Orchestrator 用 `afterEventId` 独占式游标增量轮询"自上次读取以来记录了什么"，不必重放整个 session；需要完整细节时再按轮调用 `get_session_context`。
   - 事件 ID 是当前投影中的单调 1-based 序号，历史追加时保持稳定；历史因容量上限被裁剪时投影会重新编号——调用方应对比返回中的 `totalTurns` 检测裁剪并用全量快照重新同步。未知 session 或非法游标返回 `CONTEXT_INSUFFICIENT` 形态；空会话是合法的空投影（`totalEvents: 0`）。
   - 参数：`sessionId` (必填), `afterEventId` (可选，非负整数，独占式游标，缺省为全量快照), `limit` (可选，单次返回上限，默认 200，最大 500)。
8. **`get_role_config`**
...
注入采用 **handoff 优先**策略并以 token（估算值，非字符数）为预算（默认总预算 6000 token，按源均分、单源下限 1200）：每个来源**完整内联最近一轮的结构化 handoff**（goal、关键决定、产物文件/命令、未决事项、阻塞项），更早轮次只保留一行索引（`[Turn N | role | status] goal`），并显式列出**已知失败尝试**（`Known failed attempts (do not repeat):`，逐条 `[Turn N | role] 任务摘要`），避免下游重复踩坑——该列表属于低优先级可丢弃段，超预算时先于最近一轮的任何章节被丢弃。历史条目没有 handoff 时（旧会话或 vendor 未遵循报告契约）回退为精简的 legacy 轮次回放（`Final answer` 截断到 1200 字符）。超预算时按"先丢复现细节、后丢结论"的固定顺序丢弃（更早轮次索引 → 失败尝试列表 → Tests → Open Items → Commands → Files → Blockers → Decisions → Findings → 证据 → 最终硬截断），实际丢弃的章节记录在该轮 `sharedContextAudit.droppedSections` 中，审计还记录 `strategy`（`handoff` | `legacy`）、`estimatedTokens` 与 `injectedOwnHistory`；被裁剪的信息不会丢失——`get_session_context` 可按轮取回全文；由于接收方 vendor Agent 并不拥有 AgentMesh 的 MCP 工具（R19 真实测试结论），注入指令会把取回请求路由给 Orchestrator（"申报缺失项，由 Orchestrator 经 `get_session_context` 取回"），而不是让 vendor 直接调用该工具。该轮实际注入了哪些来源仍记录在 `contextSources` 字段中。`contextSessionId` 仍是可用的单源兼容形式。会话自身的原生续接与外部来源注入是并存的：`continue_task` 有原生 Session ID 时只免除自身历史的注入，`contextSessionIds` 指定的其他来源照常注入。

**知识级 staleness（注入时标记）**：会话级新鲜度只回答"整个 Session 的树状态是否仍然匹配"，注入时会进一步做**逐轮判定**——当最近一轮记录的工作树指纹与当前树不一致（该轮证据存在且可比对），其知识性章节（Decisions / Findings / Blockers；legacy 回放中的 Findings）会被加上 `[stale knowledge: the working tree changed after this turn was recorded — revalidate before reusing these conclusions]` 前缀，把 repo 级 STALE 精细化到"哪些结论已被作废"。当前树证据不可用时**不会**伪造标记（诚实降级为会话级标签）；REWOUND 场景下被回退到的早轮不会被误标。注入指令同步告知下游该前缀的语义。

**Handoff 契约（写端）**：`delegate_task`/`continue_task` 的 worker 与 tester prompt 末尾会附加固定的 Handoff Report 契约（Reviewer 除外，其输出由严格的 PASS/FAIL findings 契约约束），要求 vendor Agent 在最终回答结尾以固定小节（`## Goal` / `## Decisions` / `## Files` / `## Commands` / `## Tests` / `## Open Items` / `## Blockers`）收尾并保持简洁；`Goal` 是 agent 自报的一句话目标，注入时优先生效（缺省回退为任务首行，而非整段任务文本）。runner 由此解析出每轮的 `handoff` 结构存入会话历史（解析失败时该轮无 `handoff`，注入自动回退 legacy 渲染）；解析会剥离列表项中的 Markdown 链接包装与反引号。长度约束在源头生效，接收端不再依赖大字符截断。契约同时明确要求：**若 SPEC 或任务文本自身矛盾或不可实现，vendor 必须最先在 Open Items 小节申报——SPEC 矛盾优先于任何实现缺陷**。**Blocker 是一等结构化字段**（`blockers: { summary, requires }`）：仅当工作无法继续时申报，每行一条 `- <阻塞内容> (requires: agent | user | resource | dependency | environment)`，`requires` 声明"卡在哪、需要谁解决"（其他 Agent 角色先行 / 用户决策或凭据 / 配额席位 / 上游任务或依赖 / 传输沙箱网络等基础设施）；vendor 漏写或写错 `requires` 标记时降级为 `user`（由人裁决），不会丢弃整条 blocker。**Reviewer 轮次的 handoff 无需额外契约**：runner 会从解析出的 PASS/FAIL 裁决与 findings 确定性派生（outcome 映射裁决、keyDecisions 记录裁决摘要、openItems 列出 findings、files 汇总涉及文件），使下游注入同样紧凑。

**Handoff 信号（读端，MCP 返回）**：`delegate_task` / `continue_task` / `review_changes` 的返回文本在存在结构化 handoff 时会紧跟 `Summary:` 附带一行摘要：

```
Handoff: goal=<截断80字>; decisions=N; files=N; commands=N; openItems=N; blockers=N
```

当 `openItems>0` 时额外追加提示行 `⚠ N open item(s) reported — review before chaining further work (see get_session_context)`；`openItems=0` 时该提示行不出现。Orchestrator 应把 `openItems>0` 视为**需要人工裁决的信号**——包括 vendor 依据契约申报的 SPEC 自身矛盾——不必逐轮调用 `get_session` 才能发现未决事项。当 `blockers>0` 时额外追加升级提示行 `⚠ N blocker(s) reported (requires: <去重后的目标>) — resolve before chaining further work (see get_session_context)`：这是**机器可读的升级路由信号**，Orchestrator 据此决定换 Agent 重派（`agent`）、停下来问用户（`user`/`resource`）、先补上游任务（`dependency`）还是修环境（`environment`），而不必从自由文本里猜。程序化调用方也可直接读取返回 `AgentResult.handoff` 结构（含 `blockers`，与会话历史中的 handoff 同源同值）。
...
│  get_session, get_session_context, get_session_events,     │
│  list_agents, ...)                                         │
└──────────────────────────────┬──────────────────────────────┘
                               │
                               ▼
...
import {
  ...
  computeTurnFreshness, // (entry, current) => 逐轮 ContextFreshness（注入 stale 标记的同一判定）
  projectSessionEvents, // (session) => SessionEvent[]：turn history 的确定性事件投影
} from "agentmesh";
```
...
- `projectSessionEvents` 把 Bridge Session 的 turn history 投影为 append-only 事件序列（`turn.recorded` / `handoff.recorded` / `findings.recorded` / `evidence.recorded` / `safety.recorded`）。它是**纯投影而非独立状态存储**：确定性派生、可重复计算，历史追加时事件序号稳定，因此可作增量游标（运行时入口为 MCP 工具 `get_session_events` 的 `afterEventId`）；历史被容量上限裁剪时投影重编号，调用方以 `totalTurns` 检测并全量重同步。payload 只携带索引与小型可操作字段（blocker 升级目标、findings 计数/严重级别、证据指纹），完整细节仍按轮经 `get_session_context` 取回。
